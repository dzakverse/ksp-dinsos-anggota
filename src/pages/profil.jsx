...
export default function Profil() {
  ...
  useEffect(() => {
    api.get('/profile')
      ...
      .catch(() => setError('Gagal memuat data profil. Coba muat ulang halaman.'))
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="flex flex-col items-center gap-3 w-full md:w-auto">
            ...
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500 text-white shadow-sm">
              <ShieldCheck size={14} />
              {data.status_keanggotaan === 'AKTIF' ? 'Anggota Aktif' : 'Anggota Nonaktif'}
            </span>
          </div>
          ...
          <div className="flex-1 w-full">
            <div className="flex items-start justify-between gap-4 pb-4 border-b border-slate-100">
              <div>
                <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 tracking-tight">{data.nama}</h2>
                <p className="text-xs font-semibold text-slate-400 mt-0.5 tracking-wider uppercase">{data.id_anggota}</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-8 pt-6">
              <div>
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Nomor Induk Kependudukan</p>
                <p className="text-base font-bold text-slate-800 mt-1">{data.nik || '-'}</p>
              </div>
              <div>
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Nomor Induk Pegawai</p>
                <p className="text-base font-bold text-slate-800 mt-1">{data.nip || '-'}</p>
              </div>
              <div>
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Tempat, Tanggal Lahir</p>
                <p className="text-base font-bold text-slate-800 mt-1">{ttl}</p>
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex items-center gap-3.5 p-3 rounded-xl bg-slate-50/70 border border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-800 mt-0.5">{data.whatsapp || '-'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3.5 p-3 rounded-xl bg-slate-50/70 border border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-800 mt-0.5">{data.email || '-'}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-[#000D21] rounded-2xl p-6 sm:p-8 text-white shadow-md relative overflow-hidden flex flex-col justify-between">
          ...
          <div>
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold tracking-tight mt-1">{formatTanggal(data.tanggal_bergabung)}</h3>
            </div>
          </div>
